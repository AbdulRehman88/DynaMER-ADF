...
def infer_ablation_modality_keys(run):
    variant = str(run.get("baseline_variant", ""))
    dataset = str(run.get("dataset", ""))
    default_keys = str(run["modality_keys"]).split("|")

    if dataset != "SEED-IV":
        return default_keys

    modality_map = {
        "v3_eeg_combined_only": ["eeg_combined"],
        "v3_eye_only": ["eye_features"],
        "v3_de_only": ["eeg_de"],
        "v3_psd_only": ["eeg_psd"],
        "v3_de_eye": ["eeg_de", "eye_features"],
        "v3_psd_eye": ["eeg_psd", "eye_features"],
    }

    return modality_map.get(variant, default_keys)


def apply_ablation_model_overrides(model_cfg, run):
    cfg = dict(model_cfg)
    variant = str(run.get("baseline_variant", ""))

    overrides = {
        "v3_path_v1_only": {"path_mode": "v1_only"},
        "v3_path_v2_only": {"path_mode": "v2_only"},
        "v3_path_fixed_mean": {"path_mode": "fixed_mean"},
        "v3_fusion_mean": {"fusion_mode": "mean"},
    ...
    cfg.update(overrides.get(variant, {}))
    cfg.setdefault("path_mode", "learned_dual")
    cfg.setdefault("fusion_mode", "gated")
    return cfg
...
def make_model(run, baseline_variant, model_cfg, device):
    run_with_variant = dict(run)
    run_with_variant["baseline_variant"] = str(baseline_variant).lower()
    ...
    return DynaMERADFAblationModel(
        ...
        path_mode=str(model_cfg.get("path_mode", "learned_dual")),
...
def run_single_training(
    ...
    model = make_model(run_with_variant, baseline_variant, cfg["model"], device)
                ...
                torch.save(
                    {
                        ...
                        "baseline_variant": baseline_variant,
                        "model_config": apply_ablation_model_overrides(cfg["model"], run_with_variant),
                        "run": run,
                        "epoch": epoch,
                        "monitor_metric": monitor_metric,
                        "monitor_value": score,
...
def main() -> int:
    ...
    plan_df.to_csv(out_dir / "19B_dynamer_v3_ablation_run_plan.csv", index=False)

    logger.info(f"Selected registry rows: {len(selected)}")
    logger.info(f"DynaMER-v3-Ablation variants: {baseline_variants}")
    logger.info(f"Total DynaMER-v3-Ablation training runs: {len(plan_df)}")
    ...
    pbar = tqdm(total=total, desc="DynaMER-v3-Ablation training", unit="run")
    ...
    for run in selected.to_dict(orient="records"):
        for baseline_variant in baseline_variants:
            try:
                ...
                logger.error(f"DynaMER-v3-Ablation run failed: baseline={baseline_variant} run={run.get('run_id')} error={repr(e)}")
                result = {
                    "baseline_variant": baseline_variant,
    ...
    add_check(checks, "DynaMER-v3-Ablation variants", baseline_variants, baseline_variants, len(baseline_variants) > 0)
    ...
    run_report_path = out_dir / "19B_dynamer_v3_ablation_training_run_report.csv"
    epoch_report_path = out_dir / "19B_dynamer_v3_ablation_all_epoch_metrics.csv"
    checks_path = out_dir / "19B_dynamer_v3_ablation_training_checks.csv"
    summary_path = out_dir / "19B_dynamer_v3_ablation_training_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            ...
            "run_plan": str(out_dir / "19B_dynamer_v3_ablation_run_plan.csv"),
    ...
    logger.info(f"Overall DynaMER-v3-Ablation training stage passed: {overall_passed}")

    print("\nStage 14 DynaMER-v3-Ablation training outputs:")
    print(f"1. {selected_path}")
    print(f"2. {out_dir / '19B_dynamer_v3_ablation_run_plan.csv'}")
    ...
    if not overall_passed:
        logger.error("DynaMER-v3-Ablation training stage failed. Inspect failed runs before continuing.")
        return 1

    logger.info("DynaMER-v3-Ablation training stage passed.")
    return 0
