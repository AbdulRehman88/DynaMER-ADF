...
def make_model(run: Dict[str, Any], baseline_variant: str, model_cfg: Dict[str, Any], device: torch.device) -> BaselineEmotionModel:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="14_train_baselines: baseline training using Stage 12 data/splits/engine.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--local-paths", required=True)
    parser.add_argument("--baseline-config", default="configs/14_train_baselines.yaml")
    ...
    logger = Logger(out_dir / "14_train_baselines_log.txt")
    logger.info("Starting 14_train_baselines.")
    ...
    plan_df.to_csv(out_dir / "14_baseline_run_plan.csv", index=False)

    logger.info(f"Selected registry rows: {len(selected)}")
    logger.info(f"Baseline variants: {baseline_variants}")
    logger.info(f"Total baseline training runs: {len(plan_df)}")
    ...
    pbar = tqdm(total=total, desc="Baseline training", unit="run")
    ...
    for run in selected.to_dict(orient="records"):
        for baseline_variant in baseline_variants:
            try:
                ...
                logger.error(f"Baseline run failed: baseline={baseline_variant} run={run.get('run_id')} error={repr(e)}")
                result = {
                    "baseline_variant": baseline_variant,
    ...
    add_check(checks, "baseline variants", baseline_variants, baseline_variants, len(baseline_variants) > 0)
    ...
    run_report_path = out_dir / "14_baseline_training_run_report.csv"
    epoch_report_path = out_dir / "14_baseline_all_epoch_metrics.csv"
    checks_path = out_dir / "14_baseline_training_checks.csv"
    summary_path = out_dir / "14_baseline_training_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            ...
            "run_plan": str(out_dir / "14_baseline_run_plan.csv"),
            "run_report": str(run_report_path),
            "epoch_metrics": str(epoch_report_path),
            "checks": str(checks_path),
            "summary": str(summary_path),
            "log": str(out_dir / "14_train_baselines_log.txt"),
    ...
    logger.info(f"Overall baseline training stage passed: {overall_passed}")

    print("\nStage 14 baseline training outputs:")
    print(f"1. {selected_path}")
    print(f"2. {out_dir / '14_baseline_run_plan.csv'}")
    print(f"3. {run_report_path}")
    print(f"4. {epoch_report_path}")
    print(f"5. {checks_path}")
    print(f"6. {summary_path}")
    print(f"7. {out_dir / '14_train_baselines_log.txt'}")
    print(f"8. {out_dir / 'runs'}")

    if not overall_passed:
        logger.error("Baseline training stage failed. Inspect failed runs before continuing.")
        return 1

    logger.info("Baseline training stage passed.")
    return 0
