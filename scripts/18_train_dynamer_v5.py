...
def make_model(run: Dict[str, Any], baseline_variant: str, model_cfg: Dict[str, Any], device: torch.device) -> DynaMERv5Model:
    return DynaMERv5Model(
        ...
        residual_init_scale=float(model_cfg.get("residual_init_scale", 0.10)),
        residual_max_scale=float(model_cfg.get("residual_max_scale", 0.35)),
    ).to(device)
...
def main() -> int:
    ...
    plan_df.to_csv(out_dir / "18_dynamer_v5_run_plan.csv", index=False)

    logger.info(f"Selected registry rows: {len(selected)}")
    logger.info(f"DynaMER-v5 variants: {baseline_variants}")
    logger.info(f"Total DynaMER-v5 training runs: {len(plan_df)}")
    ...
    pbar = tqdm(total=total, desc="DynaMER-v5 training", unit="run")
    ...
    for run in selected.to_dict(orient="records"):
        for baseline_variant in baseline_variants:
            try:
                ...
                logger.error(f"DynaMER-v5 run failed: baseline={baseline_variant} run={run.get('run_id')} error={repr(e)}")
                result = {
                    "baseline_variant": baseline_variant,
    ...
    add_check(checks, "DynaMER-v5 variants", baseline_variants, baseline_variants, len(baseline_variants) > 0)
    ...
    run_report_path = out_dir / "18_dynamer_v5_training_run_report.csv"
    epoch_report_path = out_dir / "18_dynamer_v5_all_epoch_metrics.csv"
    checks_path = out_dir / "18_dynamer_v5_training_checks.csv"
    summary_path = out_dir / "18_dynamer_v5_training_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            ...
            "run_plan": str(out_dir / "18_dynamer_v5_run_plan.csv"),
    ...
    logger.info(f"Overall DynaMER-v5 training stage passed: {overall_passed}")

    print("\nStage 14 DynaMER-v5 training outputs:")
    print(f"1. {selected_path}")
    print(f"2. {out_dir / '18_dynamer_v5_run_plan.csv'}")
    ...
    if not overall_passed:
        logger.error("DynaMER-v5 training stage failed. Inspect failed runs before continuing.")
        return 1

    logger.info("DynaMER-v5 training stage passed.")
    return 0
