...
def stable_run_seed(base_seed: int, capacity_variant: str, fold_index: int, run_id: str) -> int:
    """Deterministic per-candidate/per-fold seed independent of run order."""
    key = f"{int(base_seed)}::{str(capacity_variant).lower()}::{int(fold_index)}::{str(run_id)}"
...
CAPACITY_VARIANT_DISPLAY = {
    ...
    "v3_high_spike": "DynaMER-ADF high spike",
    "v3_moddrop_0p10": "DynaMER-ADF modality dropout 0.10",
    "v3_tcn_depth_1": "DynaMER-ADF TCN depth 1",
    "v3_tcn_depth_3": "DynaMER-ADF TCN depth 3",
    "v3_dropout_0p10": "DynaMER-ADF dropout 0.10",
    "v3_dropout_0p30": "DynaMER-ADF dropout 0.30",
}

DEFAULT_CAPACITY_VARIANTS = list(CAPACITY_VARIANT_DISPLAY.keys())
...
CAPACITY_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "v3_high_spike": {"spike_mix": 0.25},
    "v3_moddrop_0p10": {"modality_dropout": 0.10},
...
def make_model(run: Dict[str, Any], capacity_variant: str, device: torch.device) -> torch.nn.Module:
    capacity_variant = str(capacity_variant).lower()
    if capacity_variant not in CAPACITY_OVERRIDES:
        raise ValueError(f"Unsupported capacity variant: {capacity_variant}")

    cfg = dict(BASE_MODEL_CONFIG)
    cfg.update(CAPACITY_OVERRIDES[capacity_variant])
...
def run_single_training(
    ...
    capacity_variant = str(capacity_variant).lower()
    run_seed = stable_run_seed(int(base_seed), capacity_variant, fold_index, run_id)
    set_global_seed(run_seed)

    capacity_run_id = f"{capacity_variant}__{run_id}"
    run_dir = out_dir / "runs" / sanitize(capacity_variant) / sanitize(run_id)

    if run_dir.exists() and not overwrite:
        return {
            "capacity_variant": capacity_variant,
            "capacity_display": CAPACITY_VARIANT_DISPLAY.get(capacity_variant, capacity_variant),
    ...
    model = make_model(run, capacity_variant, device)
    ...
    model_config.update(CAPACITY_OVERRIDES[capacity_variant])
    ...
    for epoch in range(1, int(training_cfg["epochs"]) + 1):
        ...
        row = {
            ...
            "tcn_layers": int(model_config["tcn_layers"]),
            "modality_dropout": float(model_config["modality_dropout"]),
            "spike_mix": float(model_config["spike_mix"]),
            ...
            torch.save(
                {
                    ...
                    "model_family": "DynaMER-ADF capacity audit",
                    "capacity_variant": capacity_variant,
                    "capacity_display": CAPACITY_VARIANT_DISPLAY.get(capacity_variant, capacity_variant),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 22E: DynaMER-ADF subject-mixed 5-fold capacity audit.")
    parser.add_argument("--config", default="configs/config.yaml")
    parser.add_argument("--local-paths", default="configs/local_paths.yaml")
    parser.add_argument("--registry", default=None, help="Optional Stage 22A registry path.")
    parser.add_argument("--variants", default=",".join(DEFAULT_CAPACITY_VARIANTS), help="Comma-separated ADF capacity variants.")
    parser.add_argument("--max-runs", type=int, default=None, help="Optional smoke-test limit over registry rows.")
    ...
    out_dir = base_dir / "capacity_audit_22E"
    out_dir.mkdir(parents=True, exist_ok=True)

    logger = Logger(out_dir / "22E_train_dynamer_adf_subject_mixed_capacity_audit_log.txt")
    logger.info("Starting Stage 22E DynaMER-ADF subject-mixed capacity audit.")
    logger.info(f"Project root: {project_root}")
    logger.info(f"Output directory: {out_dir}")

    registry_path = Path(args.registry) if args.registry else base_dir / "22_seed_iv_subject_mixed_5fold_registry.csv"
    temporal_index_path = project_root / "outputs" / "temporal_views" / "08_prepare_temporal_feature_views" / "08_temporal_view_index.csv"
    split_summary_path = base_dir / "22_seed_iv_subject_mixed_5fold_summary.json"
    ...
    registry = pd.read_csv(registry_path).sort_values(["fold_index"]).reset_index(drop=True)
    ...
    if unknown:
        raise ValueError(f"Unsupported capacity variants: {unknown}. Supported: {sorted(CAPACITY_OVERRIDES)}")
    ...
    for run in registry.to_dict(orient="records"):
        for variant in variants:
            ...
            plan_rows.append({
                ...
                "capacity_display": CAPACITY_VARIANT_DISPLAY.get(variant, variant),
                ...
                "spike_mix": cfg["spike_mix"],
            })
    plan_df = pd.DataFrame(plan_rows)
    plan_path = out_dir / "22E_capacity_audit_training_plan.csv"
    plan_df.to_csv(plan_path, index=False)

    logger.info(f"Selected registry rows: {len(registry)}")
    logger.info(f"Selected capacity variants: {variants}")
    ...
    pbar = tqdm(total=len(plan_df), desc="Stage 22E capacity audit", unit="run")
    for run in registry.to_dict(orient="records"):
        for variant in variants:
            try:
                ...
                logger.error(f"Run failed: variant={variant} run={run.get('run_id')} error={repr(e)}")
                result = {
                    "capacity_variant": variant,
                    "capacity_display": CAPACITY_VARIANT_DISPLAY.get(variant, variant),
                    "capacity_run_id": f"{variant}__{run.get('run_id')}",
    ...
    run_report_path = out_dir / "22E_capacity_audit_run_report.csv"
    epoch_report_path = out_dir / "22E_capacity_audit_all_epoch_metrics.csv"
    checks_path = out_dir / "22E_capacity_audit_checks.csv"
    summary_path = out_dir / "22E_capacity_audit_summary.json"
    ...
    add_check(checks, "selected capacity variants", variants, variants, len(variants) > 0)
    add_check(checks, "planned runs", int(len(plan_df)), int(len(registry)) * int(len(variants)))
    ...
    summary = {
        "name": "22E_train_dynamer_adf_subject_mixed_capacity_audit",
        "created_at": now(),
        ...
        "outputs": {
            ...
            "log": str(out_dir / "22E_train_dynamer_adf_subject_mixed_capacity_audit_log.txt"),
        ...
        "scientific_statement": "Stage 22E audits DynaMER-ADF-family capacity under SEED-IV subject-mixed 5-fold using order-safe per-candidate/per-fold seeds, train-only standardization, validation-based checkpoint selection, and test-only final metrics. This is a diagnostic capacity audit, not the primary subject-independent claim.",
    }
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    logger.info(f"Wrote run report: {run_report_path}")
    logger.info(f"Wrote epoch metrics: {epoch_report_path}")
    logger.info(f"Overall Stage 22E passed: {overall_passed}")
    ...
    print(f"6. {out_dir / '22E_train_dynamer_adf_subject_mixed_capacity_audit_log.txt'}")
    print(f"7. {out_dir / 'runs'}")

    if not overall_passed:
        logger.error("Stage 22E failed. Inspect failed runs before summarizing.")
        return 1
    logger.info("Stage 22E passed. It is safe to run Stage 22F summary.")
    return 0
