...
from dynamer.models.dynamer_adf_model import DynaMERADFModel
...
def make_model(run: Dict[str, Any], baseline_variant: str, model_cfg: Dict[str, Any], device: torch.device) -> DynaMERADFModel:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="16_train_dynamer_adf: DynaMER-v3 training using Stage 12 data/splits/engine.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--local-paths", required=True)
    parser.add_argument("--baseline-config", default="configs/16_train_dynamer_adf.yaml")
    ...
    logger = Logger(out_dir / "16_train_dynamer_adf_log.txt")
    logger.info("Starting 16_train_dynamer_adf.")
    ...
    summary = {
        ...
        "outputs": {
            ...
            "log": str(out_dir / "16_train_dynamer_adf_log.txt"),
    ...
    print(f"7. {out_dir / '16_train_dynamer_adf_log.txt'}")
