...
from dynamer.models.dynamer_bitcn_model import DynaMERBiTCNModel
...
def make_model(run: Dict[str, Any], baseline_variant: str, model_cfg: Dict[str, Any], device: torch.device) -> DynaMERBiTCNModel:
    return DynaMERBiTCNModel(
        ...
        modality_dropout=float(model_cfg.get("modality_dropout", 0.10)),
        ...
        spike_mix=float(model_cfg.get("spike_mix", 0.15)),
    ).to(device)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="15_train_dynamer_bitcn: DynaMER-v2 training using Stage 12 data/splits/engine.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--local-paths", required=True)
    parser.add_argument("--baseline-config", default="configs/15_train_dynamer_bitcn.yaml")
    ...
    logger = Logger(out_dir / "15_train_dynamer_bitcn_log.txt")
    logger.info("Starting 15_train_dynamer_bitcn.")
    ...
    plan_df.to_csv(out_dir / "15_dynamer_v2_run_plan.csv", index=False)

    logger.info(f"Selected registry rows: {len(selected)}")
    logger.info(f"DynaMER-v2 variants: {baseline_variants}")
    logger.info(f"Total DynaMER-v2 training runs: {len(plan_df)}")
    ...
    pbar = tqdm(total=total, desc="DynaMER-v2 training", unit="run")
    ...
    for run in selected.to_dict(orient="records"):
        for baseline_variant in baseline_variants:
            try:
                ...
                logger.error(f"DynaMER-v2 run failed: baseline={baseline_variant} run={run.get('run_id')} error={repr(e)}")
                result = {
                    "baseline_variant": baseline_variant,
    ...
    add_check(checks, "DynaMER-v2 variants", baseline_variants, baseline_variants, len(baseline_variants) > 0)
    ...
    run_report_path = out_dir / "15_dynamer_v2_training_run_report.csv"
    epoch_report_path = out_dir / "15_dynamer_v2_all_epoch_metrics.csv"
    checks_path = out_dir / "15_dynamer_v2_training_checks.csv"
    summary_path = out_dir / "15_dynamer_v2_training_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            ...
            "run_plan": str(out_dir / "15_dynamer_v2_run_plan.csv"),
            "run_report": str(run_report_path),
            "epoch_metrics": str(epoch_report_path),
            "checks": str(checks_path),
            "summary": str(summary_path),
            "log": str(out_dir / "15_train_dynamer_bitcn_log.txt"),
    ...
    logger.info(f"Overall DynaMER-v2 training stage passed: {overall_passed}")

    print("\nStage 14 DynaMER-v2 training outputs:")
    print(f"1. {selected_path}")
    print(f"2. {out_dir / '15_dynamer_v2_run_plan.csv'}")
    print(f"3. {run_report_path}")
    print(f"4. {epoch_report_path}")
    print(f"5. {checks_path}")
    print(f"6. {summary_path}")
    print(f"7. {out_dir / '15_train_dynamer_bitcn_log.txt'}")
    print(f"8. {out_dir / 'runs'}")

    if not overall_passed:
        logger.error("DynaMER-v2 training stage failed. Inspect failed runs before continuing.")
        return 1

    logger.info("DynaMER-v2 training stage passed.")
    return 0
